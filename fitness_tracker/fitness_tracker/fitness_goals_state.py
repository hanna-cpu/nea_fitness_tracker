"""Backend logic for the Fitness Goals page (fitness_goals_page.py) -
setting a target for each of the four goal types.
"""
...
class FitnessGoalsState(State):
    # One text field per goal type - all kept as strings since that's what
    # a number input gives back, converted to float only when saving.
    ...
    def load_goals(self):
        """Runs when the page loads - fills in each field with the user's
        current goal, if they've already set one for that type.
        """
    ...
    # Each of these four runs when its own row's Save button is clicked.
    # db.upsert_goal() either creates the goal or overwrites the existing
    # one for that goal_type - see the comment in db.py for how that works.
