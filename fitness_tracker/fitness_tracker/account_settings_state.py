"""Backend logic for the Account Settings page (account_settings_page.py) -
viewing and editing the logged-in user's profile details.
"""
...
class AccountSettingsState(State):
    # One variable per editable field. height_cm is kept as a string (not a
    # float) because that's what a text input naturally gives back - it only
    # gets converted to a number in save(), right before it's needed.
    name: str = ""
    email: str = ""
    gender: str = ""
    height_cm: str = ""
    date_of_birth: str = ""
    message: str = ""  # green "Saved." confirmation
    error: str = ""    # red validation/error message
    ...
    def load_user(self):
        """Runs when the page loads - fetches the user's current details from
        the database and fills in the form fields with them.
        """
        ...
        if user is None:
            return
        # `or ""` turns a database NULL into an empty string, since text
        # inputs can't display None.
