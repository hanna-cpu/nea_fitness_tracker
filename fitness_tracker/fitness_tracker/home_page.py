"""Frontend for the Home page - the main dashboard shown after logging in.
Backend logic (loading goals/chart data) lives in home_state.py.
"""
...
def _card(*children: rx.Component, **props) -> rx.Component:
    """A reusable "card" wrapper - a bordered, padded box with a solid
    background - used for the notification banner styling and each of the
    four chart cards below, so they all look consistent.
    """
...
def _notification_banner(notification: rx.Var) -> rx.Component:
    """One dismissible amber banner for a single unread notification."""
    return rx.hstack(
...
def _steps_card() -> rx.Component:
    """Bar chart of steps walked per day (last 7 days), with a red dashed
    ...
    return _card(
        ...
        rx.recharts.bar_chart(
            ...
            rx.recharts.tooltip(),  # shows exact values on hover
            # .to(int) - the reference line prop only accepts whole numbers,
            # but the goal is stored as a float, so it needs converting.
...
def _workout_card() -> rx.Component:
    """Bar chart of total workout minutes per day (last 7 days) vs the goal."""
    return _card(
        rx.hstack(
...
def _weight_card() -> rx.Component:
    """Line chart of logged weight over time vs the target weight."""
    return _card(
        rx.hstack(
        ...
        rx.recharts.line_chart(
            ...
            rx.recharts.x_axis(data_key="date"),
            rx.recharts.y_axis(domain=["auto", "auto"]),  # let the y-axis scale to fit the data, not start at 0
            rx.recharts.cartesian_grid(stroke_dasharray="3 3"),
            rx.recharts.tooltip(),
...
def _calories_card() -> rx.Component:
    """Grouped bar chart comparing calories consumed vs burned, per day."""
    return _card(
        rx.hstack(
        ...
        rx.recharts.bar_chart(
            # Two separate <Bar> elements sharing one chart draws two bars
            # side by side per day, instead of stacking them.
    ...
    return rx.vstack(
        ...
        rx.center(
            rx.vstack(
                ...
                # rx.foreach loops over a list held in state, rendering one
                # component per item - here, one banner per unread notification.
                rx.vstack(
