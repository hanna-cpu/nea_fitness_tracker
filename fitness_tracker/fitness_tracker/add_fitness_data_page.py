"""Frontend for the Add Fitness Data page: four buttons (Workout/Weight/
Calories/Steps), each opening its own pop-up dialog with a small form.
Backend logic lives in add_fitness_data_state.py.
"""
...
def _field(label: str, input_component: rx.Component) -> rx.Component:
    """A small grey label above one input box - reused for every field in
    every dialog below.
    """
    return rx.vstack(
...
def _data_dialog(
    ...
    *fields: rx.Component,
    ...
    """Builds one "button that opens a pop-up form" - used four times below,
    once per category, each passing in its own icon, fields and save handler.

    rx.dialog.root is the overlay itself; `open`/`on_open_change` let the
    matching *_state.py close the dialog automatically once saving succeeds,
    instead of the dialog managing its own open/closed state.
    """
    ...
    return rx.vstack(
        ...
        rx.center(
            rx.vstack(
                ...
                # Four calls to _data_dialog, one per category - each wires
                # up its own icon, fields, and save function from the state.
                rx.grid(
                    _data_dialog(
                        "dumbbell",
                        "Workout",
