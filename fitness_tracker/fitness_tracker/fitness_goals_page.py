"""Frontend for the Fitness Goals page - one row per goal type, each with
its own Save button. Backend logic lives in fitness_goals_state.py.
"""
...
def _goal_row(icon: str, label: str, value: rx.Var, on_change, on_save, unit: str) -> rx.Component:
    """One goal's row: icon, label, input box, and its own Save button -
    saving one goal doesn't affect or need to touch the others.
    """
    return rx.hstack(
