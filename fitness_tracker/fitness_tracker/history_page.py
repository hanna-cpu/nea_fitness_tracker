"""Frontend for the History page - four tables (Workout/Weight/Calories/
Steps), built by the same _section() helper. Backend logic (turning
database rows into these tables' data) lives in history_state.py.
"""
...
def _section(icon: str, title: str, headers: list[str], rows: rx.Var, row_keys: list[str]) -> rx.Component:
    """One history table: a heading, then a table whose columns are given
    by `headers` and whose cells are pulled from each row dict using
    `row_keys` (e.g. row_keys=["date", "steps"] reads row["date"], row["steps"]).
    """
    return rx.vstack(
        rx.hstack(
        ...
        rx.table.root(
            ...
            rx.table.body(
                # rx.foreach builds one table row per item in the `rows` list,
                # using the lambda to turn each dict into table cells.
                rx.foreach(
                    rows,
